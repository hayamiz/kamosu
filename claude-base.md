...
Q&A の回答（`outputs/` 内のファイル）を wiki 記事として統合するプロトコル。
Compilation Protocol と同じ記事作成・更新手順を再利用する。

### 目的
Query で得られた知見（比較分析、新たな接続の発見、横断的サマリ等）を wiki に還元し、知識を蓄積する。

### Step 1: ソースの読み込み
promote 対象として指定されたファイルを読み取り、内容を把握する:
- 主題・トピック
- 含まれる知見・分析結果
- 参照されている wiki 記事

### Step 2: 既存 wiki の把握
Compilation Protocol の Step 3 と同じ:
1. `_master_index.md` を読み、現在のカテゴリ構造を把握する
2. 関連しそうなカテゴリの `_category/*.md` を読み、既存記事を確認する
3. 必要に応じて既存記事本体を読み、重複・関連性を確認する
...
### Step 3: 統合判断
以下の基準で統合方法を決定する:
- **既存記事への統合**: promote 対象の内容が既存記事のトピックを補強・拡張する場合
- **新規記事の作成**: 新しいトピック、または複数記事にまたがる横断的分析の場合

### Step 4: 記事の作成・更新
Compilation Protocol の Step 4 と同じ手順に従う。追加ルール:
- `sources:` フィールドに元の outputs ファイルパスを記録する（例: `outputs/analysis-2026-04-06.md`）
- 元の Query で参照された wiki 記事への `[[wikilink]]` を維持する

### Step 5: バックリンクの追加
Compilation Protocol の Step 5 と同じ。

### Step 6: インデックス更新
Compilation Protocol の Step 6 と同じ:
1. `_master_index.md` を更新
2. 影響を受けた `_category/*.md` を更新
3. `_cross_references.md` を更新

### Step 7: ログ追記
`wiki/_log.md` に promote エントリを追加する:
```markdown
## [YYYY-MM-DD] promote | <要約>
- Promoted: <ファイルリスト>
